...
{
  printf("Usage: %s [-t seconds] [-w %%age] [-c %%age] [-d decimals] [-l label] [-h]\n",
         argv[0]);
  printf("\n");
...
void display(const char *label, int used,
             int const warning, int const critical, int const decimals)
{
  ...
  printf("%*.*i%%", decimals, decimals, used);
}
...
int main(int argc, char *argv[])
{
  ...
  while (1) {
    ...
    display(label, 100 * (used - old_used) / (total - old_total),
